        ...
        if ((tp_dev.sta & 0xc000) == TP_CATH_PRES)   /* 按键按下了一次(此时按键松开了.) */
        {
            ...
            switch (cnt) {
                ...
                case 5:     /* 全部5个点已经得到 */
                    ...
                    lcd_clear(WHITE);   /* 清屏 */
                    lcd_show_string(35, 110, lcddev.width, lcddev.height, 16, "Touch Screen Adjust OK!",
                                    BLUE); /* 校正完成 */
                    delay_ms(1000);
                    tp_save_adjust_data();
...
uint8_t tp_init(void) {
    ...
    if (lcddev.id == 0X5510 || lcddev.id == 0X4342 || lcddev.id == 0X1018)  /* 电容触摸屏,4.3寸/10.1寸屏 */
    {
        ...
        return 0;
    } else if (lcddev.id == 0X1963 || lcddev.id == 0X7084 ||
               lcddev.id == 0X7016)     /* SSD1963 7寸屏或者 7寸800*480/1024*600 RGB屏 */
    {
        ft5206_init();
