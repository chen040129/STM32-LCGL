...
static void lv_chart_event(const lv_obj_class_t *class_p, lv_event_t *e) {
    ...
    if (code == LV_EVENT_PRESSED) {
        ...
        lv_event_set_ext_draw_size(e,
                                   LV_MAX4(chart->tick[0].draw_size, chart->tick[1].draw_size, chart->tick[2].draw_size,
...
static void draw_y_ticks(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, lv_chart_axis_t axis) {
    ...
    for (i = 0; i <= total_tick_num; i++) {
        ...
        int32_t tick_value = lv_map(total_tick_num - i, 0, total_tick_num, chart->ymin[sec_axis],
                                    chart->ymax[sec_axis]);
        ...
        if (major && t->label_en) {
            ...
            lv_point_t size;
            lv_txt_get_size(&size, part_draw_dsc.text, label_dsc.font, label_dsc.letter_space, label_dsc.line_space,
                            LV_COORD_MAX,
                            LV_TEXT_FLAG_NONE);
...
static void draw_x_ticks(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, lv_chart_axis_t axis) {
    ...
    lv_coord_t pad_left =
            lv_obj_get_style_pad_left(obj, LV_PART_MAIN) + lv_obj_get_style_border_width(obj, LV_PART_MAIN);
    lv_coord_t w = ((int32_t) lv_obj_get_content_width(obj) * chart->zoom_x) >> 8;
